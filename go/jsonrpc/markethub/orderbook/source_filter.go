...
// Normalize canonicalizes a source filter without mutating its slices.
//
// Returns:
...
// Validate validates an order book source filter.
//
// Returns:
//   - Validation error.
//
...
func (f SourceFilter) Validate() error {
	...
	if f.VenueCategories != nil && len(f.VenueCategories) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: venue_categories=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	if f.LiquidityModels != nil && len(f.LiquidityModels) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: liquidity_models=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	if f.AMMPoolChains != nil && len(f.AMMPoolChains) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: amm_pool_chains=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	...
	if f.AMMPoolChains != nil {
		if !contains(f.VenueCategories, VenueCategoryDEX) || !contains(f.LiquidityModels, LiquidityModelAMM) {
			return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: amm_pool_chains=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
		seen := make(map[Chain]struct{}, len(f.AMMPoolChains))
		for _, chain := range f.AMMPoolChains {
			if !isValidAMMPoolChain(chain) {
				return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: amm_pool_chain=invalid", k4k3ruSDKAppError.InvalidParameter())
			}
...
func normalizeStrings[T ~string](values []T) []T {
	...
	for index, value := range values {
		result[index] = T(strings.ToLower(strings.TrimSpace(string(value))))
	}
...
func validateVenueCategories(values []VenueCategory) error {
	...
	for _, value := range values {
		if value != VenueCategoryCEX && value != VenueCategoryDEX {
			return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: venue_category=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
...
func validateLiquidityModels(values []LiquidityModel) error {
	...
	for _, value := range values {
		if value != LiquidityModelOrderBook && value != LiquidityModelAMM {
			return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: liquidity_model=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
		if _, exists := seen[value]; exists {
			return k4k3ruSDKAppError.Tracef("failed to validate order book source filter: %w: liquidity_model=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
