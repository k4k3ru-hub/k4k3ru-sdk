...
// Normalize canonicalizes a Carry source filter.
//
// Version:
//   - 2026-09-06: Added.
...
// Validate validates a Carry source filter.
//
// Version:
//   - 2026-09-06: Added.
func (f SourceFilter) Validate() error {
	...
	if f.VenueCategories != nil && len(f.VenueCategories) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: venue_categories=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	if f.LiquidityModels != nil && len(f.LiquidityModels) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: liquidity_models=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	if f.AMMPoolChains != nil && len(f.AMMPoolChains) == 0 {
		return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: amm_pool_chains=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	if err := validateUnique(f.VenueCategories, func(v VenueCategory) bool { return v == VenueCategoryCEX || v == VenueCategoryDEX }); err != nil {
		return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: venue_category=invalid", k4k3ruSDKAppError.InvalidParameter())
	}
	if err := validateUnique(f.LiquidityModels, func(v LiquidityModel) bool { return v == LiquidityModelOrderBook || v == LiquidityModelAMM }); err != nil {
		return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: liquidity_model=invalid", k4k3ruSDKAppError.InvalidParameter())
	}
	if f.AMMPoolChains != nil {
		if !contains(f.VenueCategories, VenueCategoryDEX) || !contains(f.LiquidityModels, LiquidityModelAMM) {
			return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: amm_pool_chains=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
		if err := validateUnique(f.AMMPoolChains, func(v Chain) bool {
			...
			return k4k3ruSDKAppError.Tracef("failed to validate carry source filter: %w: amm_pool_chain=invalid", k4k3ruSDKAppError.InvalidParameter())
		}
	}
	return nil
}
