...
const carrySubscriptionNamespace = "MarketHub.Carry"
...
func (p Params) SubscriptionKey() (string, error) {
	...
	return fmt.Sprintf("%s:ac=%s:s=%s:ba=%s:q=%s:hpm=%d:mefb=%s:rf=%s:src=%s", carrySubscriptionNamespace, strings.ToUpper(string(p.AssetClass)), p.Symbol, p.BaseAsset, p.Quantity, p.HoldingPeriodMinutes, p.MinimumEstimatedFundingBps, upperJoin(p.RouteFamilies), p.sourceSelector()), nil
}
