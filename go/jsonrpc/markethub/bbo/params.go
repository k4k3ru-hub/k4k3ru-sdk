...
const (
	bboSubscriptionNamespace = "MarketHub.BBO"
	maxSymbolLength          = 16
)
...
//
// Parameters:
//   - data: JSON-encoded BBO parameters.
//
// Version:
//   - 2026-09-05: Added.
func (p *Params) UnmarshalJSON(data []byte) error {
	if p == nil {
		return k4k3ruSDKAppError.Tracef("failed to decode bbo parameters: %w: destination=null", k4k3ruSDKAppError.InvalidParameter())
	}
...
// Normalize applies stable defaults and canonical formatting to BBO parameters.
//
// Returns:
...
// Validate validates BBO parameters.
//
// Returns:
//   - Validation error.
//
...
func (p Params) Validate() error {
	...
	if p.AssetClass != AssetClassCrypto {
		return k4k3ruSDKAppError.Tracef("failed to validate bbo parameters: %w: asset_class=invalid", k4k3ruSDKAppError.InvalidParameter())
	}
	if p.MarketType != MarketTypeSpot && p.MarketType != MarketTypePerp {
		return k4k3ruSDKAppError.Tracef("failed to validate bbo parameters: %w: market_type=invalid", k4k3ruSDKAppError.InvalidParameter())
	}
	if p.Symbol == "" {
		return k4k3ruSDKAppError.Tracef("failed to validate bbo parameters: %w: symbol=empty", k4k3ruSDKAppError.InvalidParameter())
	}
	...
	if p.SourceFilter != nil {
		if err := p.SourceFilter.Validate(); err != nil {
			return k4k3ruSDKAppError.Tracef("failed to validate bbo parameters: %w", err)
		}
	}
	return nil
}

// SubscriptionKey builds a stable venue-independent BBO subscription key.
//
// Returns:
...
func invalidJSONError(err error) error {
	return k4k3ruSDKAppError.Tracef("failed to decode bbo parameters: %w: %w: json=invalid", k4k3ruSDKAppError.InvalidParameter(), err)
}
