...
// TestMessageRouterRoutesOrderBookEvent verifies subscription composition and lifecycle.
//
// Version:
//   - 2026-09-15: Added.
func TestMessageRouterRoutesOrderBookEvent(t *testing.T) {
	...
	router, err := newMessageRouter(requests, newBBOEventRegistry(), events, newSpreadEventRegistry(), newCarryEventRegistry(), newAMMPoolEventRegistry(), newAMMPoolNewPairEventRegistry())
	if err != nil {
		t.Fatal(err)
	}
...
// TestNewMessageRouterValidatesDependencies verifies subscription composition and lifecycle.
//
// Version:
//   - 2026-09-15: Added.
func TestNewMessageRouterValidatesDependencies(t *testing.T) {
	...
	if _, err := newMessageRouter(nil, newBBOEventRegistry(), newOrderBookEventRegistry(), newSpreadEventRegistry(), newCarryEventRegistry(), newAMMPoolEventRegistry(), newAMMPoolNewPairEventRegistry()); err == nil || !strings.Contains(err.Error(), "request_tracker=null") {
		t.Fatalf("nil request tracker error = %v", err)
	}
	if _, err := newMessageRouter(newRequestTracker(), nil, newOrderBookEventRegistry(), newSpreadEventRegistry(), newCarryEventRegistry(), newAMMPoolEventRegistry(), newAMMPoolNewPairEventRegistry()); err == nil || !strings.Contains(err.Error(), "bbo_event_registry=null") {
		t.Fatalf("nil bbo event registry error = %v", err)
	}
	if _, err := newMessageRouter(newRequestTracker(), newBBOEventRegistry(), nil, newSpreadEventRegistry(), newCarryEventRegistry(), newAMMPoolEventRegistry(), newAMMPoolNewPairEventRegistry()); err == nil || !strings.Contains(err.Error(), "order_book_event_registry=null") {
...
func newTestMessageRouter(t *testing.T) (*messageRouter, *requestTracker, *bboEventRegistry) {
	...
	router, err := newMessageRouter(requests, events, newOrderBookEventRegistry(), newSpreadEventRegistry(), newCarryEventRegistry(), newAMMPoolEventRegistry(), newAMMPoolNewPairEventRegistry())
	if err != nil {
